...
$this->title = "{$subsystem->subsystemTitle} - ".Yii::t('app', 'Packagingreport');
...
<span class="col1">
    <form method="get" id="filterform">
        <input type="hidden" name="r" value="subsystem/packagingreport">
       ...
       <!-- <a class="filter-element width90" href="javascript:void(lastmonth())"><?=Yii::t('app','lastmonth')?></a> -->
        ...
        <div id="otherOptions" style="display:none;">
            <span class="filter-element width90"><label><?=Yii::t('app','Packaging Category')?></label><?=Html::dropDownList('category', $post['category'], ([''=>Yii::t('app','All categories')]+$data['categoryOptions']), ['class'=>'form-control'] )?></span><br/>
            <span class="filter-element width90"><label><?=Yii::t('app','packaging_title')?></label><?=Html::textInput( 'packaging_title', $post['packaging_title'], ['class'=>'form-control width100'] )?></span><br/>
...
--><span class="col2">
    ...
    ?><div>
        ...
        $urlParameters = [
            ...
            'packaging_title' => ( isset($post['packaging_title'])?$post['packaging_title']:''),
        ...
        <tr>
            <th><a href="<?=Url::to(array_merge($urlParameters,['page'=>0,'sort'=>sortVal($urlParameters['sort'],'packaging_id'),'page'=>0]))?>"><?=Yii::t('app','packaging_id')?></a></th>
            <th><a href="<?=Url::to(array_merge($urlParameters,['page'=>0,'sort'=>sortVal($urlParameters['sort'],'packaging_title')]))?>"><?=Yii::t('app','packaging_title')?></a></th>
            <th><a href="<?=Url::to(array_merge($urlParameters,['page'=>0,'sort'=>sortVal($urlParameters['sort'],'category_title')]))?>"><?=Yii::t('app','Packaging Category')?></a></th>
            <th><a href="<?=Url::to(array_merge($urlParameters,['page'=>0,'sort'=>sortVal($urlParameters['sort'],'packaging_number')]))?>"><?=Yii::t('app','packaging_number')?></a></th>
            <th><a href="<?=Url::to(array_merge($urlParameters,['page'=>0,'sort'=>sortVal($urlParameters['sort'],'packaging_number')]))?>"><?=Yii::t('app','packaging_number')?></a></th>
        </tr>        
        <?php
        ...
        foreach($data['rows'] as $row){
            ...
            ?><tr>
                ...
                <td><?=$row['packaging_title']?></td>
                <td><?=$row['category_title']?></td>
                <td><?=$row['packaging_number']?></td>
                <td><div class="diacol" style="width:<?=3+round(300*$row['packaging_number']/max($data['maxCount'],1))?>px"></div></td>
              </tr><?php
