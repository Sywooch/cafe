...
// =============================================================================
function newOrder(order_day_sequence_number) {
    ...
    if (order_day_sequence_number) {
        try {
            data_id = parseInt(order_day_sequence_number);//+1;
        } catch (err) {
            data_id = 1;
        }
    } else {
    ...
    window.orderData =  {
        order_day_sequence_number: data_id,
        order_payment_type: '',
        discount_id: 0,
        order_total: 0,
...
function packagingClicked(event) {
    ...
    if (packaging) {
        ...
        $(document).trigger( "sell:orderupdated");
        // updateOrderTotal();
    }
...
function addPackagingToOrder(packaging) {
    // add to order
    if (window.orderData.order_packaging[packaging.packaging_id]) {
        window.orderData.order_packaging[packaging.packaging_id].count++;
    } else {
        window.orderData.order_packaging[packaging.packaging_id] = {
            count: 1,
            packaging: packaging
        };
    }
    ...
    if (orderItem.length === 0) {
        ...
        $('#orderItem' + packaging.packaging_id + 'count').html(window.orderData.order_packaging[packaging.packaging_id].count);
    }
...
function plusitem(event) {
    ...
    window.orderData.order_packaging[packaging_id].count++;
    $('#orderItem' + packaging_id + 'count').html(window.orderData.order_packaging[packaging_id].count);
...
function minusitem(event) {
    ...
    if (window.orderData.order_packaging[packaging_id].count > 0) {
        $('#orderItem' + packaging_id + 'count').html(window.orderData.order_packaging[packaging_id].count);
    } else {
        delete(window.orderData.order_packaging[packaging_id]);
...
function updateOrderTotal() {
    ...
    for (var packaging_id in window.orderData.order_packaging) {
        total += window.orderData.order_packaging[packaging_id].count * window.orderData.order_packaging[packaging_id].packaging.packaging_price;
    }
    // total -= getDiscount(false);
    total -= window.orderData.discount_value || 0;
 ...
 * 
 */
window.discountTimeout=false;

function onDiscountReceived(reply){
    if (window.orderData.discount_id) {
        window.orderData.discount_value=reply.discount_value;
        window.orderData.discount_type=reply.discount_type;
        window.orderData.discount_count=reply.discount_count;
        window.orderData.discount_message=reply.discount_message;
        window.orderData.discount_title=reply.discount_title;
        $.toast().reset('all');
        $.toast({
            position: 'top-center',
        ...
        clearDiscount(null,null,null);
        // suggest a discount
        if(reply.discount_id){
            $.toast().reset('all');
            $.toast({
                position: 'top-center',
...
function clearDiscount(xhr, ajaxOptions, thrownError) {
    window.orderData.discount_id=0;
    window.orderData.discount_value=0;
    window.orderData.discount_type='';
    window.orderData.discount_title='';
    updateOrderTotal();
}

function askDiscount(){
    jQuery.ajax('index.php?r=sell/getdiscount', {
        type: "POST",
        data: {order:window.orderData},
        dataType:'json',
        success: onDiscountReceived,
        error: clearDiscount 
    });
}
...
function getDiscount() {
    ...
    window.discountTimeout=setTimeout(askDiscount,1000);
}
...
function paid(paymentTypeName) {
    ...
    return function () {
        ...
        newOrder(window.orderData.order_day_sequence_number+1);
        $("#dialog").dialog("close");
...
$(window).load(function () {
    ...
    for (var i in discounts) {
        ...
        option = $('<option value="' + i + '">' + discounts[i] + '</option>');
        discountSelector.append(option);
    }
    ...
    $(document).on( "sell:orderupdated", updateOrderTotal);
    $(document).on( "sell:orderupdated", getDiscount);
